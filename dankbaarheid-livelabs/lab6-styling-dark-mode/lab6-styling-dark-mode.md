...
3. Set the following color values:

    | Setting | Value | Purpose |
    |---|---|---|
    | **Header Accent** (`@g_Accent-BG`) | `#8B7355` | Brown header |
    | **Body Accent** (`@g_Accent-OG`) | `#F5F0E8` | Cream page background |
    | **Primary** | `#8B7355` | Buttons, links |
...
2. Click **Create File** (or edit `app.css` if it already exists from Lab 4).
...
2. Start by duplicating the current light theme. Set these dark-mode colors:
    ...
    | Setting | Value |
    |---|---|
    | **Header Accent** (`@g_Accent-BG`) | `#3D3225` |
    | **Body Accent** (`@g_Accent-OG`) | `#1A1410` |
    | **Primary** | `#C4A265` (gold for dark mode) |
    | **Body Title** | `#E8DCC8` |
...
1. Navigate to **Shared Components** → **User Interface Attributes** → **JavaScript → File URLs** and add a reference to our JavaScript file:

    ```
    #APP_FILES#app#MIN#.js
    ```

2. Create `app.js` in **Static Application Files** with this content:

    ```javascript
    /**
     * Dankbaarheid App — Dark Mode Controller
     * Switches APEX theme style between 'Dankbaarheid Licht' and 'Dankbaarheid Donker'.
     */
    (function() {
        'use strict';

        var THEME_NUMBER = 42; // Universal Theme number in APEX
        var LIGHT_STYLE  = 'Dankbaarheid Licht';
        var DARK_STYLE   = 'Dankbaarheid Donker';

        function applyTheme(isDark) {
            // Use APEX theme API to switch style
            apex.theme.setStyle(isDark ? DARK_STYLE : LIGHT_STYLE, THEME_NUMBER);
        }

        function initDarkMode() {
            var userPref = apex.item('P0_DARK_MODE') ? apex.item('P0_DARK_MODE').getValue() : null;
        ...
        $(document).ready(initDarkMode);
    })();
    ```

    ![Placeholder: Static Application Files showing app.js uploaded](images/apex-js-upload.png " ")

3. Create an **Application Item** `P0_DARK_MODE`:
    - Navigate to **Shared Components** → **Application Items**
    - Click **Create**
    - **Name**: `P0_DARK_MODE`
    - **Session State Protection**: Unrestricted

4. Create an **Application Process** to set the dark mode preference on load:
...
5. On the Settings page (Page 30), add a **Dynamic Action** on the dark mode switch (`P30_DARK_MODE`) change event:
    - **Event**: Change
    ...
    ```javascript
    var isDark = $v('P30_DARK_MODE') === 'Y';
    apex.theme.setStyle(
        isDark ? 'Dankbaarheid Donker' : 'Dankbaarheid Licht', 42
    );
    ```

6. Add dark-mode-specific CSS overrides to `app.css`:
...
7. Run the application, go to **Instellingen**, and toggle the dark mode switch.
